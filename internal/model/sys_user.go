...
type SysUser struct {
	...
	Account       string    `gorm:"type:varchar(15);not null;comment:账号" json:"account"`
	UserName      string    `gorm:"type:varchar(10);not null;comment:用户名称" json:"user_name"`
	Password      string    `gorm:"type:varchar(12);not null;comment:密码" json:"-"`
	Avatar        string    `gorm:"type:varchar(255);comment:头像" json:"avatar"`
	Ip            string    `gorm:"type:varchar(20);not null;comment:ip地址" json:"ip"`
	Location      string    `gorm:"type:varchar(50);not null;comment:地址" json:"location"`
	...
	Status        bool      `gorm:"type:tinyint(1) unsigned;not null;comment:状态 1启用 0禁用" json:"status"`
}
